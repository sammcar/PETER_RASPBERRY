...
bool send_frame(int sockfd, sockaddr_in& serverAddr, cv::VideoCapture& cap, int frame_count) {
    ...
    if (buffer.capacity() < FRAME_WIDTH * FRAME_HEIGHT / 2) {  
        buffer.reserve(FRAME_WIDTH * FRAME_HEIGHT / 2);  // Estimación más realista del tamaño JPEG
    }
    buffer.clear();  
    ...
    static std::vector<uchar> packet;
    if (packet.capacity() < 4 + PACKET_SIZE) {
        packet.reserve(4 + PACKET_SIZE);
    }
    packet.resize(4 + PACKET_SIZE);  
    ...
    for (uint16_t i = 0; i < total_packets; i++) {
        ...
        // Copiar datos de manera eficiente
        std::copy_n(buffer.data() + start, chunk_size, packet.data() + 4);
...
int main() {
    ...
    int frame_count = 0;
    int error_count = 0;
    constexpr int MAX_ERROR_COUNT = 20;  // Reiniciar la cámara tras 10 errores consecutivos

    while (true) {
        // Si la cámara no está abierta o ha fallado varias veces, reiniciarla
        if (!cap.isOpened() || error_count >= MAX_ERROR_COUNT) {
            std::cerr << "🔄 Reiniciando la cámara..." << std::endl;
            cap.release();
            cap.open(PUERTO_CAM, cv::CAP_V4L2);

            if (!cap.isOpened()) {
                std::cerr << "❌ Error crítico: No se pudo reiniciar la cámara." << std::endl;
                return -1;
            }
        ...
        // Si no hay socket, crearlo nuevamente
        if (sockfd <= 0) {
            sockfd = setup_udp_socket(serverAddr);
        ...
        if (!send_frame(sockfd, serverAddr, cap, frame_count)) {
            error_count++;
            if (error_count >= MAX_ERROR_COUNT) {
                std::cerr << "⚠️ Demasiados errores al capturar frames, reiniciando..." << std::endl;
                continue;  // Forzar reinicio
            }
        } else {
            error_count = 0;  // Resetear contador si se envió correctamente
        }
